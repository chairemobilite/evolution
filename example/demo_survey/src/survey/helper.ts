...
const householdMembersSectionComplete = function (interview) {
    ...
    if (household.size > odSurveyHelper.countPersons({ interview })) {
        return false;
    }
...
const profileInfoForPersonComplete = function (person, interview) {
    ...
    if (person.usualWorkPlaceIsHome !== true && _isWorker) {
        ...
        if (_isBlank(usualWorkPlaceGeometry)) {
            return false;
        }
    } else if (_isWorker && _isBlank(person.usualWorkPlaceIsHome)) {
        return false;
    }
    if (_isStudent) {
        const usualSchoolPlaceGeometry = _get(person, 'usualSchoolPlace.geometry.coordinates');
        if (_isBlank(usualSchoolPlaceGeometry)) {
            return false;
        }
    }
    return true;
...
const tripsIntroForPersonComplete = function (person, interview) {
    ...
    if (
        ...
        ((person.didTripsOnTripsDate === 'yes' || person.didTripsOnTripsDate === true) &&
            (_isBlank(journeys[0]) || _isBlank(journeys[0].departurePlaceType)))
    ) {
        return false;
    }
    return true;
...
const visitedPlacesForPersonComplete = function (person, interview) {
    ...
    if (!tripsIntroForPersonComplete(person, interview)) {
        return false;
    }
    if (person.didTripsOnTripsDate !== 'yes' && person.didTripsOnTripsDate !== true) {
        return true;
    }
    ...
    for (const visitedPlaceId in visitedPlaces) {
        ...
        if (
            ...
            (visitedPlace._sequence > 1 &&
                visitedPlace._sequence < countVisitedPlaces &&
                (_isBlank(visitedPlace.arrivalTime) || _isBlank(visitedPlace.departureTime))) ||
...
const tripsForPersonComplete = function (person, interview) {
    ...
    const journey = odSurveyHelper.getJourneysArray({ person })[0];
    if (!visitedPlacesForPersonComplete(person, interview)) {
        return false;
    }
    ...
    for (const tripId in trips) {
        ...
        for (const segmentId in segments) {
            ...
            if (_isBlank(segment._sequence) || _isBlank(segment.mode)) {
                return false;
            }
        }
    }
...
const travelBehaviorForPersonComplete = function (person, interview) {
    ...
    if (!tripsForPersonComplete(person, interview)) {
        return false;
    }
    const household = surveyHelperNew.getResponse(interview, 'household') as any;
    if (household.size > 1 && _isBlank(person.whoAnsweredForThisPerson)) {
        return false;
    }
    return true;
};
...
const allPersonsTripsAndTravelBehaviorComplete = function (interview) {
    ...
    for (const personId in persons) {
        const person = persons[personId];
        if (!travelBehaviorForPersonComplete(person, interview)) {
            return false;
        }
    }
    return true;
