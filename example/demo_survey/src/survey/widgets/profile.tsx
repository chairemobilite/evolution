...
export const personWorkOnTheRoad = {
    ...
    label: {
        fr: function (interview, path) {
            ...
            const genderString = helper.getGenderString(
                person,
                'livreuse, représentante, conductrice, policière',
                'livreur, représentant, chauffeur, policier',
                'livreur/se, représentant(e), conducteur/trice, policier/ère',
                'livreur/se, représentant(e), conducteur/trice, policier/ère'
            );
...
export const personWorkAtHomeNumberOfDaysPerWeek = {
    ...
    conditional: function (interview, path) {
        const workAtHomeAtLeastOnceAWeek = surveyHelperNew.getResponse(
            interview,
            path,
            null,
            '../workAtHomeAtLeastOnceAWeek'
        );
        ...
        return [
            {
                validation:
                    _isBlank(value) &&
                    surveyHelperNew.getResponse(interview, path, null, '../workAtHomeAtLeastOnceAWeek') === 'yes',
                errorMessage: {
                    fr: 'Cette réponse est requise',
                    en: 'This field is required.'
                }
...
export const personUsualWorkPlaceGeography = {
    ...
    label: {
        fr: function (interview, path) {
            ...
            const pretext =
                householdSize === 1
                    ? `Veuillez localiser sur la carte votre lieu habituel de travail${isOnTheRoadWorker ? ' (lieu de départ de vos déplacements sur la route)' : ''} sur la carte`
                    : `Veuillez localiser le lieu habituel de travail de ${surveyHelperNew.getResponse(interview, path, null, '../nickname')}${isOnTheRoadWorker ? ' (lieu de départ des déplacements sur la route)' : ''}`;
            return `${pretext} <br />
      <span class="_pale _oblique">
        ...
        en: function (interview, path) {
            ...
            const pretext =
                householdSize === 1
                    ? `Please locate your usual work place on the map${isOnTheRoadWorker ? ' (departure location for work-related trips)' : ''}`
                    : `Please locate ${surveyHelperNew.getResponse(interview, path, null, '../nickname')}'s usual work place on the map${isOnTheRoadWorker ? ' (departure location for work-related trips)' : ''}`;
            return `${pretext} <br />
      <span class="_pale _oblique">
    ...
    geocodingQueryString: function (interview, path) {
        return surveyHelperNew.formatGeocodingQueryStringFromMultipleFields([
            surveyHelperNew.getResponse(interview, path, null, '../usualWorkPlaceName')
        ]);
    },
    conditional: function (interview, path) {
        const workAtHome = surveyHelperNew.getResponse(interview, path, null, '../usualWorkPlaceIsHome');
...
export const personUsualSchoolPlaceGeography = {
    ...
    label: {
        fr: function (interview, path) {
            ...
            const pretext =
                householdSize === 1
                    ? 'Veuillez localiser votre lieu habituel d\'études sur la carte'
                    : `Veuillez localiser le lieu habituel d’études de ${surveyHelperNew.getResponse(interview, path, null, '../nickname')} sur la carte`;
            return `${pretext} <br />
      <span class="_pale _oblique">
        Vous pouvez naviguer, zoomer, cliquer sur la carte ou déplacer l'icône 
        ...
        en: function (interview, path) {
            ...
            const pretext =
                householdSize === 1
                    ? 'Please locate your usual school place on the map'
                    : `Please locate ${surveyHelperNew.getResponse(interview, path, null, '../nickname')}'s usual school place on the map`;
            return `${pretext} <br />
      <span class="_pale _oblique">
      You can navigate, zoom, click on map or drag the icon marker
    ...
    geocodingQueryString: function (interview, path) {
        return surveyHelperNew.formatGeocodingQueryStringFromMultipleFields([
            surveyHelperNew.getResponse(interview, path, null, '../usualSchoolPlaceName')
        ]);
    },
    conditional: function (interview, path) {
        const occupation = surveyHelperNew.getResponse(interview, path, null, '../occupation');
...
`;
    ...
    conditional: function (interview, path) {
        ...
        if (_isBlank(age) || (!_isBlank(age) && (age < 5 || !(age > 0)))) {
            return [false, null];
        }
        return [true, null];
    },
