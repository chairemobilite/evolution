...
export const householdMembers: GroupConfig = {
    ...
    name: {
        fr: function (groupedObject: any, sequence) {
            return `Personne ${sequence || groupedObject['_sequence']} ${groupedObject.nickname ? `• **${groupedObject.nickname}**` : ''}`;
        },
        en: function (groupedObject: any, sequence) {
            return `Person ${sequence || groupedObject['_sequence']} ${groupedObject.nickname ? `• **${groupedObject.nickname}**` : ''}`;
        }
    },
...
export const personOccupation = {
    ...
    choices: [
        ...
        },
        {
            ...
            label: {
                fr: function (interview, path) {
                    return (surveyHelperNew.getResponse(interview, path, null, '../age') as any) < 12
                        ? 'École'
        ...
        },
        {
            ...
            conditional: function (interview, path) {
                if (config.includePartTimeStudentOccupation === false) {
                    return false;
                }
        ...
        },
        {
            ...
            conditional: function (interview, path) {
                if (config.includeWorkerAndStudentOccupation === false) {
                    return false;
                }
                return (surveyHelperNew.getResponse(interview, path, null, '../age') as any) >= 16;
            }
    ...
    conditional: function (interview, path) {
        ...
        if ((person && _isBlank(person.age)) || !(person.age >= 0)) {
            return [false, null];
        } else if (person && person.age < 5) {
...
export const personCarsharingMember = {
    ...
    conditional: function (interview, path) {
        const drivingLicenseOwnership = surveyHelperNew.getResponse(
            interview,
            path,
            null,
            '../drivingLicenseOwnership'
        );
        if (_isBlank(drivingLicenseOwnership)) {
            return [false, null];
        }
        if (!_isBlank(drivingLicenseOwnership) && drivingLicenseOwnership !== 'yes') {
            return [false, 'nonApplicable'];
        }
        return [true, null];
    },
