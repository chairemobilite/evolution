...
export const personAge = {
  ...
  path: "age",
  inputType: "string",
  disallowedCharactersRegex: /\D/g, // Remove everything but numbers
  datatype: "integer",
  twoColumns: true,
  containsHtml: true,
...
export const activePersonTitle = {
  type: "text",
  align: 'center',
  containsHtml: true,
  classes: '',
  text: {
    fr: function(interview, path) {
      ...
      const activePersonId = surveyHelperNew.getResponse(interview, '_activePersonId', null);
      const activePersonNickname = activePersonId ? surveyHelperNew.getResponse(interview, `household.persons.${activePersonId}.nickname`, null) : null;
      if (!_isBlank(activePersonNickname))
      {
        return `<p class="_orange center _em" style="margin-bottom: 0;">Entrevue de <strong>${activePersonNickname}</strong></p>`;
      }
      else
      {
        return `<p class="_orange center _em" style="margin-bottom: 0;">Entrevue de <strong>${activePersonId}</strong></p>`; 
      }
    },
    en: function(interview, path) {
      const householdSize = surveyHelperNew.getResponse(interview, 'household.size', null);
      ...
      if (!_isBlank(activePersonNickname))
      {
        return `<p class="_orange center _em" style="margin-bottom: 0;"><strong>${activePersonNickname}</strong>'s interview</p>`;
      }
      else
      {
        return `<p class="_orange center _em" style="margin-bottom: 0;"><strong>${activePersonId}</strong>'s interview</p>`; 
      }
    }
  }
};

export const buttonSwitchPerson = {
  type: "button",
  align: 'center',
  containsHtml: true,
  color: 'blue',
  ...
  confirmPopup: {
    ...
    content: {
      ...
      en: function(interview) {
        return `Please fill out this section before switching persons.`;
      }
    },
    showConfirmButton: false,
    cancelButtonColor: 'blue',
