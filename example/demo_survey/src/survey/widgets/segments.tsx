...
export const segmentSubwayStationStart = {
    ...
    choices: function (interview, path) {
        ...
        if (trip) {
            ...
            const originGeometry = origin
                ? odSurveyHelper.getVisitedPlaceGeography({ visitedPlace: origin, interview, person })
                : null;
            if (originGeometry) {
                ...
                choices = _choices.sort((stationA, stationB) => {
...
export const segmentSubwayStationEnd = {
    ...
    choices: function (interview, path) {
        ...
        if (trip) {
            ...
            const destinationGeometry = destination
                ? odSurveyHelper.getVisitedPlaceGeography({ visitedPlace: destination, interview, person })
                : null;
    ...
    validations: function (value, customValue, interview, path, customPath) {
        ...
        return [
            ...
            },
            {
                validation:
                    !_isBlank(value) &&
                    !_isBlank(departureSubwayStation) &&
                    departureSubwayStation === value &&
                    departureSubwayStation !== 'dontKnow' &&
                    departureSubwayStation !== 'other',
                errorMessage: {
    ...
    validations: function (value, customValue, interview, path, customPath) {
        ...
        return [
            ...
            },
            {
                validation:
                    !_isBlank(value) &&
                    !_isBlank(departureTrainStation) &&
                    departureTrainStation === value &&
                    departureTrainStation !== 'dontKnow' &&
                    departureTrainStation !== 'other',
                errorMessage: {
            ...
            if (trip) {
                ...
                if (!_isBlank(tripTransferCategory)) {
                    ...
                    if (tripTransferCategory.category === 'private_public') {
                        switch (previousMode) {
                        case 'bicycle':
                            previousModeStr = isAlone
                                ? `${firstFormVerb} stationné ou embarqué votre vélo`
                                : `${nickname} ${firstFormVerb} stationné ou embarqué son vélo`;
                            break;
                        case 'carDriver':
                            previousModeStr = isAlone
                                ? `${firstFormVerb} stationné votre véhicule`
                                : `${nickname} ${firstFormVerb} stationné son véhicule`;
                            break;
                        case 'motorcycle':
                            previousModeStr = isAlone
                                ? `${firstFormVerb} stationné votre moto`
                                : `${nickname} ${firstFormVerb} stationné sa moto`;
                            break;
                        case 'carPassenger':
                            previousModeStr = isAlone
                                ? `${secondFormVerb} débarqué${genderString2} du véhicule`
                                : `${nickname} ${secondFormVerb} débarqué${genderString2} du véhicule`;
                            break;
                        case 'taxi':
                            previousModeStr = isAlone
                                ? `${secondFormVerb} débarqué${genderString2} du taxi`
                                : `${nickname} ${secondFormVerb} débarqué${genderString2} du taxi`;
                            break;
                        default:
                            previousModeStr = isAlone
                                ? `${secondFormVerb} débarqué${genderString2} du véhicule`
                        ...
                        case 'bicycle':
                            nextModeStr = isAlone
                                ? `${firstFormVerb} récupéré ou réutilisé votre vélo`
                                : `${nickname} ${firstFormVerb} récupéré ou réutilisé son vélo`;
                            break;
                        case 'carDriver':
                            nextModeStr = isAlone
                                ? `${firstFormVerb} récupéré votre véhicule`
                                : `${nickname} ${firstFormVerb} récupéré son véhicule`;
                            break;
                        case 'motorcycle':
                            nextModeStr = isAlone
                                ? `${firstFormVerb} récupéré votre moto`
                                : `${nickname} ${firstFormVerb} récupéré sa moto`;
                            break;
                        case 'carPassenger':
                            nextModeStr = isAlone
                                ? `${secondFormVerb} embarqué${genderString2} dans le véhicule`
                                : `${nickname} ${secondFormVerb} embarqué${genderString2} dans le  véhicule`;
                            break;
                        case 'taxi':
                            nextModeStr = isAlone
                                ? `${secondFormVerb} embarqué${genderString2} dans le taxi`
                                : `${nickname} ${secondFormVerb} embarqué${genderString2} dans le taxi`;
                            break;
                        default:
                            nextModeStr = isAlone
                                ? `${firstFormVerb} récupéré votre véhicule`
    ...
    defaultCenter: function (interview, path) {
        ...
        if (typeof originVisitedPlaceId === 'string') {
            ...
            if (originVisitedPlace) {
                const originGeography = odSurveyHelper.getVisitedPlaceGeography({
