...
export const VisitedPlacesSection: React.FC<SectionProps> = (props: SectionProps) => {
    ...
    const addVisitedPlace = (sequence, path, e) => {
        ...
        props.startAddGroupedObjects(1, sequence, path, [], (interview) => {
            ...
            if (lastVisitedPlace && sequence === lastVisitedPlace._sequence) {
                // we are inserting a new visited place at the end
    ...
    for (const _personId in persons) {
        ...
        const visitedPlaces =
            _currentJourney !== undefined ? odSurveyHelper.getVisitedPlacesArray({ journey: _currentJourney }) : [];
        ...
        for (let i = 0, count = visitedPlaces.length; i < count; i++) {
            ...
            const visitedPlaceSchedule = (
                <div
                    ...
                    onClick={
                        props.interview.allWidgetsValid && person._uuid === _personId && !selectedVisitedPlaceId
        ...
        if (atLeastOneCompletedVisitedPlace) {
            const personSchedule = (
                <div className="survey-visited-places-schedule-person-container" key={_personId}>
                    {
                        <p className={_personId === person._uuid ? ' _orange' : ''}>
                            {t('survey:person:dayScheduleFor', {
                                nickname: _person.nickname,
                                count: householdSize as number
                            })}
                        </p>
                    }
    ...
    for (let i = 0, count = visitedPlaces.length; i < count; i++) {
        ...
        const visitedPlaceItem = (
            <li
                className={`no-bullet survey-visited-place-item${visitedPlace._uuid === selectedVisitedPlaceId ? ' survey-visited-place-item-selected' : ''}`}
                        ...
                        {confirmDeleteVisitedPlace === visitedPlacePath && (
                            <div>
                                <ConfirmModal
                                    ...
                                    text={surveyHelper.parseString(
                                        personVisitedPlacesConfig.deleteConfirmPopup.content[i18n.language] ||
                                    ...
                                    title={
                                        ...
                                        personVisitedPlacesConfig.deleteConfirmPopup.title[i18n.language]
                                    ...
                                    cancelAction={null}
                                    confirmAction={() =>
                                        deleteVisitedPlace(person, visitedPlacePath, visitedPlace, visitedPlaces)
                                    }
    ...
    return (
        <section className={`survey-section survey-section-shortname-${props.shortname}`}>
            <div className="survey-section__content">
                ...
                <div className="survey-visited-places-list-and-map-container">
                    ...
                    >
                        ...
                        <li className="no-bullet" key="survey-visited-places-list-last-visited-place-not-home-question">
                            {widgetsComponentsByShortname['personLastVisitedPlaceNotHome']}
                        </li>
                        {visitedPlaces.length > 1 && (
                            <li className="no-bullet" key="survey-visited-places-list-confirm-button">
                                {!selectedVisitedPlaceId &&
            ...
            {process.env.APP_NAME === 'survey' && props.user && props.user.is_admin === true && (
                <div className="center">
                    ...
                    >
                        {t('survey:visitedPlace:resetVisitedPlaces')}
                    </button>
                </div>
            )}
        </section>
