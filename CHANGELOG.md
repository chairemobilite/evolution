...
- **Generator conditionals context tokens**: Conditional `path` values can now include `${currentJourney}`, `${currentTrip}`, and `${currentVisitedPlace}` tokens (in addition to `${currentPerson}`) so generated conditionals can target the “current” OD survey objects (fixes [#1081](https://github.com/chairemobilite/evolution/issues/1081)).
...
- OD survey helpers: new helper functions available directly in survey: 
  ...
  - `getCurrentJourneyId`, `getCurrentTripId`, `getCurrentVisitedPlaceId` to extract current context ids from a widget path or from the interview’s active ids
  - `getVisitedPlaceDescription`
  - `isUsualActivity`
- Added a `visitedPlaces` section configuration, which includes activity, location, shortcuts, next place category questions, as well as save/cancel/delete buttons with the  `VisitedPlacesSectionFactory` class (fixes [#1437](https://github.com/chairemobilite/evolution/issues/1437))
