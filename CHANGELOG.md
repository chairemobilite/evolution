...
- Admin: clearing a force approve no longer leaves the object approved, `sv_review_decisions.decision_value` becoming nullable so that an override is stored without a vote ([#1891](https://github.com/chairemobilite/evolution/pull/1891))
...
- **BREAKING**: Age thresholds in survey project configuration must now be nested under `ages` instead of top-level keys (for example, use `ages: { adultAge: 18, drivingLicenseAge: 16 }` instead of `adultAge: 18`). Vehicle limits must be nested under `vehicles` (for example, `vehicles: { maxCarsPerHouseholdMember: 3 }`). See `example/demo_survey/config.js` for the updated structure.
