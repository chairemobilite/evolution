...
# Note: Tests for helpers/survey_definition_checks.py (reusable per-field checks).
...
class TestValidTsIdentifier:
    def test_accepts_simple_name(self):
        assert survey_definition_checks.valid_ts_identifier("householdSize", {}) is True

    def test_accepts_underscore_and_dollar_prefixes(self):
        assert survey_definition_checks.valid_ts_identifier("_privateName", {}) is True
        assert survey_definition_checks.valid_ts_identifier("$specialName", {}) is True

    def test_rejects_name_starting_with_digit(self):
        assert survey_definition_checks.valid_ts_identifier("1stQuestion", {}) is False

    def test_rejects_name_with_spaces(self):
        assert (
            survey_definition_checks.valid_ts_identifier("household size", {}) is False
        )
...
class TestValidPathChars:
    def test_accepts_simple_path(self):
        assert survey_definition_checks.valid_path_chars("household.size", {}) is True
    ...
    def test_accepts_curly_brace_tokens(self):
        ...
        assert (
            survey_definition_checks.valid_path_chars(
    ...
    def test_rejects_spaces(self):
        assert survey_definition_checks.valid_path_chars("household size", {}) is False
...
class TestNonBlank:
    def test_accepts_non_empty_string(self):
        assert survey_definition_checks.non_blank("hello", {}) is True

    def test_accepts_non_string_values(self):
        assert survey_definition_checks.non_blank(0, {}) is True
        assert survey_definition_checks.non_blank(False, {}) is True

    def test_rejects_empty_string(self):
        assert survey_definition_checks.non_blank("", {}) is False
...
class TestEndsWithUnderscore:
    def test_accepts_value_ending_with_underscore(self):
        assert survey_definition_checks.ends_with_underscore("h_", {}) is True

    def test_rejects_value_not_ending_with_underscore(self):
        assert survey_definition_checks.ends_with_underscore("h", {}) is False

    def test_rejects_underscore_in_the_middle_only(self):
        assert survey_definition_checks.ends_with_underscore("h_m", {}) is False


class TestValidConditionalName:
    def test_accepts_generated_conditional_name(self):
        assert (
            survey_definition_checks.valid_conditional_name(
    ...
    def test_rejects_name_not_ending_with_conditional(self):
        assert (
            survey_definition_checks.valid_conditional_name("hasHouseholdSize1", {})
            is False
        )
