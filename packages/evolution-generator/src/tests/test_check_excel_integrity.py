...
class TestCheckExcelIntegrity:
    ...
    def test_valid_file_returns_true(self):
        ...
        create_mocked_excel_data(
            "Conditionals",
            list(CheckExcelIntegrity.CONDITIONALS_EXPECTED_HEADERS),
            [["cond1", "", "some.path", "===", "42", ""]],
        )
        try:
...
class TestCheckConditionalsSheet:
    ...
    GOOD_HEADERS = list(CheckExcelIntegrity.CONDITIONALS_EXPECTED_HEADERS)
            ...
            pytest.param(
                {
                    ...
                    "expected_message": "Sheet with name Conditionals does not exist",
                },
            ...
            pytest.param(
                {
                    ...
                    "rows": [GOOD_ROW],
                    "expected_result": False,
                    "expected_message": "Missing expected header in Conditionals sheet: conditional_name",
                },
            ...
            pytest.param(
                {
                    ...
                    "expected_result": False,
                    "expected_message": "Unbalanced parentheses for conditional_name 'cond1' in row 2: too many ')' (closing parenthesis without matching opening).",
                },
            ...
            pytest.param(
                {
                    ...
                    "expected_result": False,
                    "expected_message": "Invalid logical_operator in row 2: first row of a conditional must have empty logical_operator, got '&&'",
                },
    ...
    def test_check_conditionals_sheet_cases(self, case, capsys):
        ...
        try:
            ...
            # When validation fails, the script prints the error; we assert the exact message.
            captured = capsys.readouterr()
            if case["expected_message"] is None:
                assert (
                    "An error occurred with check_conditionals_sheet"
                    not in captured.out
                )
            else:
...
class TestValidateConditionalsRow:
    ...
    def test_missing_required_field_raises(self):
        ...
        assert "Required field is missing in row 2" in str(exc_info.value)
        assert "conditional_name" in str(exc_info.value)
    ...
    def test_invalid_conditional_name_raises(self):
        ...
        assert "Invalid conditional_name in row 2" in str(exc_info.value)
        assert "must be a non-empty string, got 10" in str(exc_info.value)

    def test_invalid_path_raises(self):
        """path must be a non-empty string."""
        with pytest.raises(Exception) as exc_info:
            self.checker._validate_conditionals_row(self._row(path=5), self.ROW_NUMBER)
        assert "Invalid path in row 2" in str(exc_info.value)
        assert "must be a non-empty string, got 5" in str(exc_info.value)
    ...
    def test_invalid_logical_operator_raises(self):
        ...
        assert "Invalid logical_operator in row 2" in str(exc_info.value)
        assert "must be '||', '&&', or empty, got 'OR'" in str(exc_info.value)
    ...
    def test_invalid_comparison_operator_raises(self):
        ...
        assert "Invalid comparison_operator in row 2" in str(exc_info.value)
    ...
    def test_invalid_parentheses_raises(self):
        ...
        assert "Invalid parentheses in row 2" in str(exc_info.value)
        assert "must be '(', ')', or empty, got '(('" in str(exc_info.value)

    def test_invalid_value_type_raises(self):
        """value must be int, float, str, or None (e.g. not date)."""
        with pytest.raises(Exception) as exc_info:
            self.checker._validate_conditionals_row(
                self._row(value=datetime.date(2024, 1, 1)), self.ROW_NUMBER
            )
        assert "Invalid value in row 2" in str(exc_info.value)
        assert "must be a number or a string" in str(exc_info.value)
...
class TestValidateConditionalsParenthesesBalance:
    ...
    def test_unbalanced_too_many_closing_raises(self):
        ...
        with pytest.raises(Exception) as exc_info:
            self.checker._validate_conditionals_parentheses_balance(row_data)
        assert "Unbalanced parentheses for conditional_name 'cond1' in row 2" in str(
            exc_info.value
        )
        assert "too many ')' (closing parenthesis without matching opening)" in str(
            exc_info.value
        )
    ...
    def test_unbalanced_unclosed_opening_raises(self):
        ...
        with pytest.raises(Exception) as exc_info:
            self.checker._validate_conditionals_parentheses_balance(row_data)
        assert "Unbalanced parentheses for conditional_name 'cond1'" in str(
            exc_info.value
        )
        assert "unclosed opening parenthesis/parentheses" in str(exc_info.value)
    ...
    def test_same_name_non_consecutive_unbalanced_raises(self):
        ...
        with pytest.raises(Exception) as exc_info:
            self.checker._validate_conditionals_parentheses_balance(row_data)
        assert "Unbalanced parentheses for conditional_name 'condA'" in str(
            exc_info.value
        )
        assert "unclosed opening parenthesis/parentheses" in str(exc_info.value)
...
class TestValidateConditionalsFirstRowNoLogicalOperator:
    ...
    def test_second_group_first_row_has_operator_raises(self):
        ...
        with pytest.raises(Exception) as exc_info:
            self.checker._validate_conditionals_first_row_no_logical_operator(row_data)
        assert "Invalid logical_operator in row 4" in str(exc_info.value)
    ...
    def test_same_name_comes_back_later_first_row_has_operator_raises(self):
        ...
        with pytest.raises(Exception) as exc_info:
            self.checker._validate_conditionals_first_row_no_logical_operator(row_data)
        assert "Invalid logical_operator in row 2" in str(exc_info.value)
        assert "condA" in str(exc_info.value) or "first row of a conditional" in str(
            exc_info.value
        )
