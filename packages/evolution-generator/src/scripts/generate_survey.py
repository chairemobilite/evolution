...
def generate_survey(config_path):
    ...
    # Call the generate_UI_tests function to generate the common-tests-helpers-template.ts if script enabled
    if enabled_generate_UI_tests:
        UI_tests_output_file_path = os.path.join(
            survey_folder_path, "tests", "common-tests-helpers-template.ts"
        )
