...
# Function to generate common-tests-helpers-template.ts
def generate_UI_tests(input_file: str, output_file: str):
    try:
        ...
        ts_code += f"{INDENT}hasTrips: boolean;\n"
        ts_code += f"}};\n\n"
        ...
        for row in list(sheet.rows)[1:]:
            ...
            if section != current_section:
                ...
                ts_code += f"export const complete{current_section}SectionTests = ({{ context, householdSize, hasTrips }}: CommonTestParametersModify) => {{\n"

                # Add verifyNavBarButtonStatus at the start of the section
                ts_code += f"{INDENT}// Verify the {section} navigation is active\n"
                ts_code += f"{INDENT}testHelpers.verifyNavBarButtonStatus({{ context, buttonText: '{section}', buttonStatus: 'active', isDisabled: false }});\n\n"

                # Add a section progress bar test for the section
                ts_code += f"{INDENT}// Progress bar test for {section} section\n"
                ts_code += f"{INDENT}testHelpers.sectionProgressBarTest({{ context, sectionName: '{section}', completionPercentage: 0 }});\n\n"
        ...
        print(f"Error with common-tests-helpers-template.ts: {e}")
        raise e
