...
/**
 * Whether a status stands in the way of approving an object.
 * Approving or rejecting an object is not refused because of another object. The admin UI
 * hides the approve control when a parent is rejected.
 * @param effectiveStatus - Effective review status of the object
 * @returns True when the status must be settled before a force approve makes sense
 */
