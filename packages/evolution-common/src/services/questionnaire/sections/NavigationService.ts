...
export class NavigationService {
    ...
    // same for navigation.
    private isSectionEnabledAndVisible(interview: UserRuntimeInterviewAttributes, section: NavigationSection): boolean {
    ...
    private findFirstNonSkippedSection(
        ...
        while (!visited.has(sectionToUrlPath(lastVisitedSection))) {
            ...
            const targetSection = this.handleIterationContextOnSection(
                { sectionShortname: targetSectionName, iterationContext: lastVisitedSection.iterationContext },
                interview
            );
            // See if it is possible to go to this section. If not,
            // continue navigation.
            if (!this.isSectionEnabledAndVisible(interview, targetSection)) {
            ...
            if (
                ...
                !this.isSectionEnabledAndVisible(interview, targetSectionWithRepeated)
            ) {
    ...
    private maybeGetTargetFromRepeatedBlock = (
        ...
        if (targetSectionConfig.type === 'repeatedBlock') {
            return this.getTargetSectionFromRepeatedBlock(interview, originSection, targetSectionConfig, direction);
        }
    ...
    private findFirstVisibleSectionInRepeatedBlock(
        ...
        for (let i = startIndex; i < sections.length; i++) {
            ...
            if (
                this.isSectionEnabledAndVisible(interview, {
     ...
     * to navigate
     * @param {NavigationSection} currentSection The current navigation section
    ...
    public navigate(
        ...
        if (direction === 'forward') {
            if (!this.isSectionCompleted(interview, currentSection)) {
                return {
                    targetSection: currentSection
                };
            }
        ...
        return this.handleSectionEntryExit(
            interview,
            this._navigate(interview, currentSection, direction),
            currentSection
        );
    }
    ...
    private _initNavigationState(
        ...
        if (this.isSectionEnabledAndVisible(interview, targetSectionWithContext)) {
            const [targetSectionWithRepeated, updatedValues] = this.maybeGetTargetFromRepeatedBlock(
                interview,
            ...
            if (
                ...
                const finalUpdatedValues = this.getValuesToUpdate(interview, targetSectionWithRepeated);
                return {
     ...
     * or the first one.
     * @param {NavigationSection} [currentSection] The current section, if any.
     */
    public initNavigationState(
        ...
        return this.handleSectionEntryExit(
            interview,
            this._initNavigationState(interview, requestedSection),
            currentSection
        );
    }
}
