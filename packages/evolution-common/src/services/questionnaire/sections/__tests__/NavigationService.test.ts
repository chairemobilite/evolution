...
const hhShouldSectionBeSkipped = jest.fn().mockReturnValue(false);
const simpleSectionsConfig: SurveySections = getAndValidateSurveySections({
    ...
    householdMembers: {
        ...
        nextSection: 'end',
        widgets: ['hhWidget'],
        isSectionSkipped: hhShouldSectionBeSkipped,
...
describe('Simple direct navigation, each section in nav bar', () => {
    ...
    describe('initNavigationState',() => {
        ...
        test('should navigate to last visited section if there are previous navigations', () => {
            ...
            const nextSectionResult = navigationService.initNavigationState(testInterview);
            expect(nextSectionResult).toEqual({
                navigationState: {
                    currentSection: {
                        sectionShortname: expectedSection,
        ...
        test('should navigate to requested section, even with previous navigations that reaches later', () => {
            ...
            const nextSectionResult = navigationService.initNavigationState(testInterview, requestedSection);
            expect(nextSectionResult).toEqual({
                navigationState: {
                    currentSection: {
                        sectionShortname: expectedSection,
        ...
        test('should navigate to the requested section, but keep current navigation', () => {
            const requestedSection = 'home';
            const expectedSection = 'home';
            ...
            // Prepare the previous navigations tate
            ...
            const nextSectionResult = navigationService.initNavigationState(testInterview, requestedSection, _cloneDeep(navigationState));
            expect(nextSectionResult).toEqual({
                navigationState: {
                    currentSection: {
                        sectionShortname: expectedSection,
    ...
    describe('Navigate function', () => {
        ...
        test('should navigate to the next section forward', () => {
            ...
            // Navigate to specific section
            const nextSectionResult = navigationService.navigate(testInterview, _cloneDeep(navigationState), 'forward');
            expect(nextSectionResult).toEqual({
                navigationState: {
                    currentSection: {
                        sectionShortname: expectedSection,
        ...
        test('should navigate to the next section backward', () => {
            ...
            const navigationState = {
                ...
                navigationHistory: [{ sectionShortname: 'home' }]
            }

            // Navigate to specific section
            const nextSectionResult = navigationService.navigate(testInterview, _cloneDeep(navigationState), 'backward');
            expect(nextSectionResult).toEqual({
                navigationState: {
                    currentSection: {
                        sectionShortname: expectedSection,
        ...
        test('should remain in last section when navigating forward in last section', () => {
            ...
            const navigationState = {
                ...
                navigationHistory: [{ sectionShortname: 'home' }, { sectionShortname: 'householdMembers' }]
            }
        ...
        test('should skip section if section is marked as such', () => {
            ...
            hhShouldSectionBeSkipped.mockReturnValueOnce(true);
            ...
            // Navigate to specific section
            const nextSectionResult = navigationService.navigate(testInterview, _cloneDeep(navigationState), 'forward');
            expect(hhShouldSectionBeSkipped).toHaveBeenCalledWith(testInterview, undefined);
            expect(hhSectionCompleted).toHaveBeenCalledWith(testInterview, undefined);
            expect(nextSectionResult).toEqual({
                navigationState: {
                    currentSection: {
                    ...
                    navigationHistory: [navigationState.currentSection]
                }
            });
        });
    })
...
describe('With repeated sections per household members, skipping select in natural flow', () => {
    ...
    describe('Navigate function', () => {
        ...
        test('should go to next section in block with same iteration if not skipped', () => {           
            ...
            expect(tbIsSectionSkipped).toHaveBeenCalledWith(testInterview, ['personId1']);
        });
        ...
        test('should go to next iteration if last section of block is skipped in natural flow', () => {           
            ...
            tbIsSectionSkipped.mockReturnValueOnce(true);
        ...
        test('should always select the correct next section for a completed interview', () => {
            ...
            tbIsSectionSkipped.mockReturnValueOnce(false);
            tbIsSectionSkipped.mockReturnValueOnce(true);
            ...
            const initialNavigation = navigationService.initNavigationState(testInterview);
            expect(initialNavigation).toEqual({
                navigationState: {
                    currentSection: expectedSectionSequence[0],
                    navigationHistory: []
                }
            });

            // Prepare the previous navigations tate
            let navigationState = initialNavigation.navigationState;

            for (let i = 1; i < expectedSectionSequence.length; i++) {
                // Add section completion data to the interview
                (testInterview.response._sections as any)[navigationState.currentSection.sectionShortname] = {
                    _startedAt: i * 10,
                    _isCompleted: true,
                };
                if (navigationState.currentSection.iterationContext) { 
                    (testInterview.response._sections as any)[navigationState.currentSection.sectionShortname][navigationState.currentSection.iterationContext[0]] = {
                        _startedAt: i * 10,
                        _isCompleted: true,
                    };
                }
                (testInterview.response._sections as any)._actions.push({
                    section: navigationState.currentSection.sectionShortname,
                    iterationContext: navigationState.currentSection.iterationContext,
                ...
                expect(nextSectionResult).toEqual({
                    ...
                    valuesByPath: expectedSection.iterationContext === undefined && navigationState.currentSection.iterationContext === undefined ? undefined : { 'response._activePersonId': expectedSection.iterationContext ? expectedSection.iterationContext[0] : undefined }
                });
                navigationState = nextSectionResult.navigationState;
            }
        });
...
describe('navigate function, further use cases', () => {
    ...
    describe('test iteration context order', () => {
        ...
        test('random, with random sequence already set', () => {
            ...
            // Navigate to specific section
            const nextSectionResult = navigationService.navigate(testInterviewWithRandomSequence, _cloneDeep(navigationState), 'forward');
