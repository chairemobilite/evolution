...
describe('getAndValidateSurveySections', () => {
    ...
    ]).test('should warn for undefined sections/errors in configuration: %s', (_location, sections, expectedWarning) => {
        jest.spyOn(global.console, 'warn');
        getAndValidateSurveySections(sections);
        expect(console.warn).toHaveBeenCalledWith(expectedWarning);
    });

    test('should throw error if repeated block section does not have a next section', () => {
        const sectionConfig: SurveySectionsConfig = {
            section1: {
                title: { en: 'Test Section', fr: 'Section de test' },
                previousSection: null,
                nextSection: null,
                widgets: ['widget1'],
                repeatedBlock: {
                    repeatForObjects: { type: 'builtin' as const, path: 'interviewablePersons' as const },
                    activeSurveyObjectPath: '_activePersonId',
                    selectionSectionId: 'section2',
                    sections: ['section3'],
                }
        ...
        expect(() => getAndValidateSurveySections(sectionConfig)).toThrow('Section "section1" cannot have a nextSection set to null when it is a repeated block.');
    })
});