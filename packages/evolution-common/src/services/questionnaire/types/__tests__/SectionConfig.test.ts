...
describe('getAndValidateSurveySections', () => {
    ...
    test('should set default values when title is provided', () => {
        const sectionWithTitle: SurveySectionsConfig = {
            section1: { title: { en: 'Test Section', fr: 'Section de test' }, previousSection: null, nextSection: null, widgets: ['widget1'] }
        };
        ...
        expect(result.section1).toEqual({
            ...
            nextSection: null,
            widgets: ['widget1'],
            navMenu: { type: 'inNav', menuName: { en: 'Test Section', fr: 'Section de test' } }
        });
    });
    ...
    test('should preserve custom navMenu when provided', () => {
        ...
        expect(result.section1.navMenu).toEqual({ type: 'hidden', parentSection: 'mainSection' });
    });

    test('should handle multiple sections correctly', () => {
        const multipleSections: SurveySectionsConfig = {
            section1: { title: { en: 'Section 1', fr: 'Section 1' }, previousSection: null, nextSection: 'section2', widgets: ['widget1'] },
            section2: {
                title: { en: 'Section 2', fr: 'Section 2' },
                previousSection: 'section1',
                nextSection: null,
                widgets: ['widget2'],
                navMenu: { type: 'inNav', menuName: { en: 'Custom Menu Name', fr: 'Nom de menu personnalisé' } }
            }
        };
        ...
        expect(result.section1.navMenu).toEqual({ type: 'inNav', menuName: { en: 'Section 1', fr: 'Section 1' } });
        expect(result.section2.navMenu).toEqual({ type: 'inNav', menuName: { en: 'Custom Menu Name', fr: 'Nom de menu personnalisé' } });
    });
    ...
    test('should preserve all properties in a complete section config', () => {
        ...
        const completeSection: SurveySectionsConfig = {
            intro: { title: { en: 'Introduction', fr: 'Introduction' }, previousSection: null, nextSection: 'section1', widgets: ['widget1'] },
            section1: {
                title: { en: 'Complete Section', fr: 'Section complète' },
                previousSection: 'intro',
                nextSection: 'outro',
                widgets: ['widget1', 'widget2'],
                navMenu: { type: 'hidden', parentSection: 'intro' },
            ...
            outro: { title: { en: 'Outro', fr: 'Conclusion' }, previousSection: 'section1', nextSection: null, widgets: ['widget3'] }
        };

        const result = getAndValidateSurveySections(completeSection);

        expect(result.section1).toEqual({ sectionName: 'section1', ...completeSection.section1, type: 'section' });
    });
        ...
        expect(result).toEqual(
            expect.objectContaining({
                section3: expect.objectContaining({
                    ...
                    type: 'section',
                    navMenu: { type: 'hidden', parentSection: 'section1' },
        ...
        each([
            ...
            [
                'repeatedBlock, missing section',
                {
                    ...
                    section3: { title: { en: 'Test Section 3', fr: 'Section de test 3' }, previousSection: null, nextSection: 'section1' }
                },
