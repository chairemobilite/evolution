...
export type SectionConfig = {
     ...
     * section should be skipped, use the `isSectionSkipped` instead.
     *
     * FIXME: As we are moving towards a config-only section definition, this
     * function should be replaced with something that can be defined in config
     *
    ...
    /**
     * Whether this section should be skipped or not. If this section should be
     * skipped on some conditions, for example if there are no widgets to show,
     * this function should return `true`. If not set, the section will not be
     * skipped.
     *
     * FIXME: As we are moving towards a config-only section definition, this
     * function should be replaced with something that can be defined in config
     ...
     */
    isSectionSkipped?: SectionConditionalFunction;
...
export const getAndValidateSurveySections = (sections: SurveySectionsConfig): SurveySections => {
    ...
    if (!firstSection) {
        console.warn('No first section defined. Make sure at least one section has "previousSection" set to null.');
    }
    const lastSection = Object.keys(sectionsWithDefaults).find(
        (sectionName) => sectionsWithDefaults[sectionName].nextSection === null
    );
    if (!lastSection) {
        console.warn('No last section defined. Make sure at least one section has "nextSection" set to null.');
    }
