...
import interviewUserIsAuthorized from '../services/auth/authorization';
...
router.get('/survey/createInterview', async (req: Request, res: Response) => {
    try {
        if (!req.user) {
        ...
        return res.status(200).json({ status: 'failed', interview: null, error: 'cannot create interview:' + error });
    }
});
...
router.get('/survey/activeInterview/', async (req: Request, res: Response) => {
    try {
...
router.get(
    ...
    interviewUserIsAuthorized(['update', 'read']),
...
router.post(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            if (interview) {
                if (req.user && (req.user as any).id !== interview.user_id) {
