...
const sendReviewMutationErrorResponse = (res: Response, error: unknown): boolean => {
    ...
    if (exportedError.errorCode === CANNOT_FORCE_APPROVE_NOTHING_TO_OVERRIDE_ERROR_CODE) {
        res.status(409).json({
            status: 'error',
...
router.get(
    ...
    async (req: Request, res: Response) => {
        ...
        if (req.params.interviewUuid) {
            try {
                const interview = await Interviews.getInterviewByUuidWithParticipant(req.params.interviewUuid);
                if (interview) {
