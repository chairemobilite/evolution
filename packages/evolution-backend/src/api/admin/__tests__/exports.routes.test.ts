...
jest.mock('chaire-lib-backend/lib/utils/filesystem/directoryManager', () => ({ directoryManager: { getAbsolutePath: jest.fn() } }));

// Mock the file manager
jest.mock('chaire-lib-backend/lib/utils/filesystem/fileManager', () => ({ fileManager: { fileExistsAbsolute: jest.fn() } }));
...
const fileExistsAbsoluteMock = fileManager.fileExistsAbsolute as jest.MockedFunction<typeof fileManager.fileExistsAbsolute>;
const getAbsolutePathMock = directoryManager.getAbsolutePath as jest.MockedFunction<typeof directoryManager.getAbsolutePath>;
...
describe('prepareCsvFileForExportBySurveyObject route', () => {
    it('Should prepare corrected response by default', async () => {
        ...
        expect(response.body).toEqual({ status: 'exportStarted' });
    ...
    it('Should prepare corrected response, if specified', async () => {
        const response = await request(app).get('/api/admin/data/prepareCsvFileForExportBySurveyObject?responseType=correctedIfAvailable');
        expect(response.status).toBe(200);
        expect(response.body).toEqual({ status: 'exportStarted' });
        expect(exportAllToCsvBySurveyObjectMock).toHaveBeenCalledWith({ responseType: 'correctedIfAvailable' });
    });
    ...
    it('Should prepare corrected response, if invalid value is specified', async () => {
        const response = await request(app).get('/api/admin/data/prepareCsvFileForExportBySurveyObject?responseType=unknownType');
        expect(response.status).toBe(200);
        expect(response.body).toEqual({ status: 'exportStarted' });
        expect(exportAllToCsvBySurveyObjectMock).toHaveBeenCalledWith({ responseType: 'correctedIfAvailable' });
    });
    ...
    it('Should prepare participant data, if specified', async () => {
        const response = await request(app).get('/api/admin/data/prepareCsvFileForExportBySurveyObject?responseType=participant');
        expect(response.status).toBe(200);
        expect(response.body).toEqual({ status: 'exportStarted' });
...
describe('downloadSurveyQuestionnaireListTxt route', () => {
    ...
    it('Should return 404 if the file does not exist', async () => {
        ...
        expect(response.body).toEqual({ status: 'notFound', message: 'file does not exist for language: en' });
    });
});
