...
export async function createPersonsForHousehold(
    ...
    for (const [personUuid, personAttributes] of sortedPersonEntries) {
        ...
        if (projectConfig.surveyObjectParsers?.person) {
            projectConfig.surveyObjectParsers.person(personAttributes);
        }

        const person = Person.create(
            _omit(personAttributes, ['workPlaces', 'schoolPlaces', 'journeys', 'vehicles']) as {
                [key: string]: unknown;
            }
        );
        ...
        if (isOk(person)) {
            ...
            person.result.assignColor(personIndex);
            household.members.push(person.result);
            personIndex++;
        } else {
            console.log(
                `========== Person ${personUuid} creation failed with errors count: ${person.errors?.length || 0} ==========`
            );
            surveyObjectsWithErrors.errorsByObject.personsByUuid[personUuid] = person.errors;
        }
    }
}
