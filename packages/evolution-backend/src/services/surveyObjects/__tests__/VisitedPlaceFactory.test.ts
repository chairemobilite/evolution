...
jest.mock('evolution-common/lib/services/baseObjects/VisitedPlace', () => ({ VisitedPlace: { create: jest.fn() } }));
...
describe('VisitedPlaceFactory', () => {
    ...
    beforeEach(() => {
        ...
        journey = { _uuid: 'journey-uuid', addVisitedPlace: jest.fn() } as unknown as Journey;

        person = { _uuid: 'person-uuid', addVisitedPlace: jest.fn() } as unknown as Person;

        journeyAttributes = {
            _uuid: 'journey-uuid',
            visitedPlaces: {
                'vp-1': { _uuid: 'vp-1', _sequence: 1, activity: 'work', name: 'Office' },
                'vp-2': { _uuid: 'vp-2', _sequence: 2, activity: 'home', name: 'Home' }
            }
        } as unknown as ExtendedJourneyAttributes;

        home = { _uuid: 'home-uuid', geography: { type: 'Point', coordinates: [-73.5, 45.5] } } as unknown as Home;
    ...
    describe('populateVisitedPlacesForJourney', () => {
        it('should create visited places successfully and add them to journey', async () => {
            ...
            const mockVisitedPlace2 = { _uuid: 'vp-2', activity: 'home', place: { geography: null } } as unknown as VisitedPlace;

            (MockedVisitedPlace.create as jest.Mock)
                .mockReturnValueOnce(createOk(mockVisitedPlace1))
            ...
            expect(MockedVisitedPlace.create).toHaveBeenCalledWith(
                expect.objectContaining({ _uuid: 'vp-1', _sequence: 1, activity: 'work', name: 'Office' }),
                surveyObjectsRegistry
            );
            expect(MockedVisitedPlace.create).toHaveBeenCalledWith(
                expect.objectContaining({ _uuid: 'vp-2', _sequence: 2, activity: 'home', name: 'Home' }),
                surveyObjectsRegistry
            );
        ...
        it('should assign home geography to home activity visited places', async () => {
            ...
            journeyAttributes.visitedPlaces = { 'vp-home': { _sequence: 1, _uuid: 'vp-home', activity: 'home' } } as any;
        ...
        it('should not assign home geography to non-home activities', async () => {
            ...
            journeyAttributes.visitedPlaces = { 'vp-work': { _sequence: 1, _uuid: 'vp-work', activity: 'work' } } as any;
        ...
        it('should skip visited places with undefined uuid', async () => {
            journeyAttributes.visitedPlaces = {
                undefined: { _uuid: 'undefined', activity: 'work' },
                'vp-1': { _uuid: 'vp-1', activity: 'home' }
            } as any;

            (MockedVisitedPlace.create as jest.Mock).mockReturnValue(createOk({ _uuid: 'vp-1' } as VisitedPlace));
        ...
        it('should sort visited places by sequence', async () => {
            ...
            journeyAttributes.visitedPlaces = {
                'vp-3': { _uuid: 'vp-3', _sequence: 3, activity: 'shopping' },
                'vp-1': { _uuid: 'vp-1', _sequence: 1, activity: 'home' },
                'vp-2': { _uuid: 'vp-2', _sequence: 2, activity: 'work' }
            } as any;

            (MockedVisitedPlace.create as jest.Mock)
                .mockReturnValueOnce(createOk({ _uuid: 'vp-1' } as VisitedPlace))
                .mockReturnValueOnce(createOk({ _uuid: 'vp-2' } as VisitedPlace))
        ...
        it('should handle visited place without place property', async () => {
            const mockVisitedPlace = { _uuid: 'vp-home', activity: 'home', place: null } as unknown as VisitedPlace;

            (MockedVisitedPlace.create as jest.Mock).mockReturnValue(createOk(mockVisitedPlace));

            journeyAttributes.visitedPlaces = { 'vp-home': { _uuid: 'vp-home', activity: 'home' } } as any;
