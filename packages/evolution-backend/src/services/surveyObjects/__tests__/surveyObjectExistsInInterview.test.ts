...
const organizationMapUuid = uuidV4();
const vehicleUuid = uuidV4();
const personVehicleUuid = uuidV4();
...
const interview = {
    ...
    corrected_response: {
        ...
        organizations: { [organizationMapUuid]: {} },
        household: {
            _uuid: householdUuid,
            _vehicles: [{ _uuid: vehicleUuid }],
            persons: {
                [personUuid]: {
                    _vehicles: [{ _uuid: personVehicleUuid }],
...
describe.each([
    ...
    ['organization', organizationMapUuid, true],
    ['vehicle', vehicleUuid, true],
    ['vehicle', personVehicleUuid, true],
...
test('does not fall back to response when corrected_response is blank', () => {
    // Reviews apply to the corrected response only; callers must populate it first.
    const interviewWithResponseOnly = {
        uuid: interviewUuid,
        response: interview.corrected_response
    } as unknown as InterviewAttributes;
    expect(surveyObjectExistsInInterview(interviewWithResponseOnly, 'person', personUuid)).toBe(false);
});
...
test('matches plain collection keys as well as underscore-prefixed keys', () => {
    const plainVehicleUuid = uuidV4();
    const plainTripChainUuid = uuidV4();
    const plainJunctionUuid = uuidV4();
    const plainWorkPlaceUuid = uuidV4();
    const plainSchoolPlaceUuid = uuidV4();
    const plainPersonUuid = uuidV4();

    const interviewWithPlainCollectionKeys = {
        uuid: interviewUuid,
        corrected_response: {
            household: {
                vehicles: [{ _uuid: plainVehicleUuid }],
                persons: {
                    [plainPersonUuid]: {
                        workPlaces: [{ _uuid: plainWorkPlaceUuid }],
                        schoolPlaces: [{ _uuid: plainSchoolPlaceUuid }],
                        journeys: {
                            [journeyUuid]: {
                                tripChains: [{ _uuid: plainTripChainUuid }],
                                trips: {
                                    [tripUuid]: {
                                        junctions: [{ _uuid: plainJunctionUuid }]
                                    }
                                }
                            }
                        }
    ...
    expect(surveyObjectExistsInInterview(interviewWithPlainCollectionKeys, 'vehicle', plainVehicleUuid)).toBe(true);
    expect(surveyObjectExistsInInterview(interviewWithPlainCollectionKeys, 'tripChain', plainTripChainUuid)).toBe(
        true
    );
    expect(surveyObjectExistsInInterview(interviewWithPlainCollectionKeys, 'junction', plainJunctionUuid)).toBe(true);
    expect(surveyObjectExistsInInterview(interviewWithPlainCollectionKeys, 'workPlace', plainWorkPlaceUuid)).toBe(true);
    expect(surveyObjectExistsInInterview(interviewWithPlainCollectionKeys, 'schoolPlace', plainSchoolPlaceUuid)).toBe(
        true
    );
});
