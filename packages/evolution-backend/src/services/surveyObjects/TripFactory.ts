...
export async function createTripsForJourney(
    ...
    for (const [tripUuid, tripAttributes] of sortedTripEntries) {
        ...
        if (projectConfig.surveyObjectParsers?.trip) {
            projectConfig.surveyObjectParsers.trip(tripAttributes);
        }
        ...
        if (isOk(trip)) {
            ...
            await createSegmentsForTrip(surveyObjectsWithErrors, trip.result, tripAttributes as ExtendedTripAttributes);
        } else {
            console.log(
