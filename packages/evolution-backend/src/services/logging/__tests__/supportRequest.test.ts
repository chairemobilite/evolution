...
jest.mock(
    'chaire-lib-backend/lib/services/mailer/transport',
    () => nodemailerMock.createTransport({ sendmail: true, newline: 'unix', path: '/usr/sbin/sendmail' }) as any as Transporter
);
...
describe('sendSupportRequestEmail function', () => {
    test('sends email to single support address', async () => {
        ...
        // Execute
        await sendSupportRequestEmail({ message, userEmail, interviewId });
    ...
    test('sends emails to multiple support addresses for same language', async () => {
        ...
        // Execute
        await sendSupportRequestEmail({ message });
    ...
    test('sends emails to different languages', async () => {
        ...
        // Execute
        await sendSupportRequestEmail({ message, userEmail });
    ...
    test('throws error when no support emails configured', async () => {
        ...
        // Execute and verify
        await expect(sendSupportRequestEmail({ message: 'Test message' })).rejects.toThrow('No support email addresses provided');
    });
    ...
    test('handles no user email gracefully', async () => {
        ...
        // Execute
        await sendSupportRequestEmail({ message, interviewId });
    ...
    test('includes currentUrl in email when provided', async () => {
        ...
        // Execute
        await sendSupportRequestEmail({ message, currentUrl });
    ...
    test('includes all parameters when provided', async () => {
        ...
        // Execute
        await sendSupportRequestEmail({ message, userEmail, interviewId, currentUrl });
