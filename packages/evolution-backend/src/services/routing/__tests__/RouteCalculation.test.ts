...
const mockedAccessibilityMapFromTransition = transitAccessibilityMapFromTransitionApi as jest.MockedFunction<
    typeof transitAccessibilityMapFromTransitionApi
>;
...
describe('calculateTimeDistanceByMode: invalid parameters', () => {
    ...
    test('No scenario, but transit requested', async () => {
        ...
        await expect(calculateTimeDistanceByMode(['walking', 'transit'], validParameters)).rejects.toThrow(
            'Transit mode requested without a scenario'
        );
    });
});
...
describe('calculateTimeDistanceByMode with Transition', () => {
    beforeEach(() => {
        projectConfig.transitionApi = { url: 'http://transition', username: 'user', password: 'password' };
    });
    ...
    test('Return calculation results', async () => {
        ...
        const routingResults = { walking: { status: 'success' as const, distanceM: 1000, travelTimeS: 600, source: 'transitionApi' } };
...
describe('getTransitSummary: with Transition', () => {
    ...
    test('Return calculation results', async () => {
        ...
        const routingResults = { status: 'success' as const, nbRoutes: 0, lines: [], source: 'transitionApi' };
