...
type ArbitraryObject = { a: number; b: string; c: boolean };
...
describe('auditObject', () => {
    ...
    it('should audit object and return audits', async () => {
        ...
        expect(results).toEqual([
            { errorCode: 'error-code-with-minimal-info', objectUuid: arbitraryUuid, objectType: 'test', version: 2 },
            {
            ...
            { errorCode: 'error-code-without-version', objectUuid: arbitraryUuid, objectType: 'test', message: 'OtherMessageKey', version: 1 }
        ]);
    });
});
