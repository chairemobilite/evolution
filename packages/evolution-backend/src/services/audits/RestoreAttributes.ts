...
export function restoreAttributes(
    ...
    surveyObjectPath: string | undefined,
    ...
    if (attributesToRestore && attributesToRestore[surveyObjectType]) {
        for (const attribute of attributesToRestore[surveyObjectType]) {
            if (surveyObjectPath) {
                if (
                    correctedResponse[surveyObjectPath][attribute] !== undefined &&
                    originalResponse[surveyObjectPath][attribute] === undefined
                ) {
                    correctedResponse[surveyObjectPath][attribute] = originalResponse[surveyObjectPath][attribute];
                }
            } else {
                if (correctedResponse[attribute] !== undefined && originalResponse[attribute] === undefined) {
