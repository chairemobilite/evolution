...
In Evolution, an **audit check** is a post-submission validation run over a completed or partially completed interview during the review process. Each failing warning/error or activated info check produces an `AuditForObject` record that is persisted in `sv_audits` and shown to reviewers in the admin app.
