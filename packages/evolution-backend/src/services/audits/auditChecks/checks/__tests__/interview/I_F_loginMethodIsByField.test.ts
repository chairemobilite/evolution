...
describe('I_F_loginMethodIsByField audit check', () => {
    const validUuid = uuidV4();

    test.each([
        { description: 'login method is byField', loginMethod: 'byField' as InterviewLoginMethod, shouldInfo: true },
        { description: 'login method is email', loginMethod: 'email' as InterviewLoginMethod, shouldInfo: false },
        { description: 'login method is unknown', loginMethod: 'unknown' as InterviewLoginMethod, shouldInfo: false },
        ...
        const result = interviewAuditChecks.I_F_loginMethodIsByField(context);
        ...
        expect(result).toMatchObject({
            ...
            errorCode: 'I_F_loginMethodIsByField',
            version: 1,
            level: 'info',
            message: 'Login method is by field',
            ignore: false
        });
    });
});
