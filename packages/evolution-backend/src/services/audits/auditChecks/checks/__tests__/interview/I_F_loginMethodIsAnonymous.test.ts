...
describe('I_F_loginMethodIsAnonymous audit check', () => {
    const validUuid = uuidV4();

    test.each([
        { description: 'login method is anonymous', loginMethod: 'anonymous' as InterviewLoginMethod, shouldInfo: true },
        { description: 'login method is email', loginMethod: 'email' as InterviewLoginMethod, shouldInfo: false },
        { description: 'login method is google', loginMethod: 'google' as InterviewLoginMethod, shouldInfo: false },
        ...
        const result = interviewAuditChecks.I_F_loginMethodIsAnonymous(context);
        ...
        expect(result).toMatchObject({
            ...
            errorCode: 'I_F_loginMethodIsAnonymous',
            version: 1,
            level: 'info',
            message: 'Login method is anonymous',
            ignore: false
        });
    });
});
