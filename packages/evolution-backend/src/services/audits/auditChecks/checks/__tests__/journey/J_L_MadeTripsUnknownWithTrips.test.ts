...
describe('J_L_MadeTripsUnknownWithTrips audit check', () => {
    ...
    test.each([
        ...
        },
        {
            ...
            journey: { didTrips: didTripsDontKnow, visitedPlaces: onePlace, trips: [], _skipTripDiary: true },
            shouldError: false
        },
        {
        ...
        },
        {
            ...
            journey: { didTrips: didTripsDontKnow, visitedPlaces: [], trips: oneTrip },
            shouldError: true
        }
    ])('$description', ({ journey, shouldError }) => {
        const context = createContextWithJourney(journey, validUuid);
        const result = journeyAuditChecks.J_L_MadeTripsUnknownWithTrips(context);
