...
describe('I_F_loginMethodIsEmail audit check', () => {
    const validUuid = uuidV4();

    test.each([
        { description: 'login method is email', loginMethod: 'email' as InterviewLoginMethod, shouldInfo: true },
        ...
        const result = interviewAuditChecks.I_F_loginMethodIsEmail(context);
        ...
        expect(result).toMatchObject({
            ...
            errorCode: 'I_F_loginMethodIsEmail',
            version: 1,
            level: 'info',
            message: 'Login method is email',
            ignore: false
        });
    });
});
