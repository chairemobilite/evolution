...
describe('I_M_Languages audit check', () => {
    it('should error when no languages', () => {
        const interview = createRealMockInterview({ _languages: undefined });
    ...
    it('should pass when languages present', () => {
        const interview = createRealMockInterview({ _languages: ['en'] });
        const context: InterviewAuditCheckContext = { interview };
        const result = interviewAuditChecks.I_M_Languages(context);
        expect(result).toBeUndefined();
    });

    it('should error when empty array (edge)', () => {
        const interview = createRealMockInterview({ _languages: [] });
        const context: InterviewAuditCheckContext = { interview };
        const result = interviewAuditChecks.I_M_Languages(context);
        expect(result).toMatchObject({ errorCode: 'I_M_Languages' });
    });
});
...
describe('I_I_DateOutsideRange audit check', () => {
    ...
    beforeAll(() => {
        projectConfig.startDate = '2025-09-02';
        projectConfig.endDate = '2025-12-16';
    });
