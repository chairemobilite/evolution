...
jest.mock('../../../models/paradataEvents.db.queries', () => ({ getParadataStream: jest.fn().mockImplementation(() => new ObjectReadableMock([])) }));
...
describe('exportInterviewLogTask', () => {
    ...
    const commonInterviewData = {
        ...
        interview_is_completed: true,
        user_id: null
    };
    const commonInterviewDataInRows = {
        id: '1',
        uuid: 'uuid',
        updated_at: '2024-10-11 09:02:00',
        is_valid: 'true',
    ...
    const logs: { [key: string]: any }[] = [
        {
            ...
            values_by_path: {
                ...
                'validations.home.geography': true,
        ...
        },
        {
            ...
            values_by_path: {
                'response.household.carNumber': 1,
                'response.household.bikeNumber': 10,
                'validations.household.carNumber': false,
                'validations.household.bikeNumber': true
            },
    ...
    test('Test default, all updates, no values', async () => {
        ...
        for (let i = 0; i < logs.length; i++) {
            expect(logRows[i]).toEqual(expect.objectContaining({ ...commonInterviewDataInRows }));
            const modifiedKeys = Object.entries(logs[i].values_by_path)
                .filter(([key, value]) => value !== null && !key.startsWith('validations.'))
                .map(([key, value]) => key)
                .join('|');
            const initializedKeys = Object.entries(logs[i].values_by_path)
                .filter(([key, value]) => value === null && !key.startsWith('validations.'))
                .map(([key, value]) => key)
                .join('|');
            ...
            expect(logRows[i].unsetFields).toEqual(
                logs[i].unset_paths !== undefined ? logs[i].unset_paths.filter((path: string) => !path.startsWith('validations.')).join('|') : ''
            );
    ...
    test('Test only participant response, no values', async () => {
        ...
        for (let i = 0; i < logs.length - 1; i++) {
            ...
            const modifiedKeys = Object.entries(logs[i].values_by_path)
                ...
                .filter(([key, value]) => key.startsWith('response.'))
                .map(([key, value]) => key)
                .join('|');
            const initializedKeys = Object.entries(logs[i].values_by_path)
                .filter(([key, value]) => value === null)
            ...
            expect(logRows[i].initializedFields).toEqual(initializedKeys);
            expect(logRows[i].unsetFields).toEqual(
                logs[i].unset_paths !== undefined ? logs[i].unset_paths.filter((key) => key.startsWith('response.')).join('|') : ''
            );
            expect(logRows[i].widgetType).toEqual('');
            expect(logRows[i].widgetPath).toEqual('');
        }
    ...
    test('Test only participant response, with values', async () => {
        ...
        const expectedCount = logs.reduce(
            (acc, log) =>
                acc +
                Object.keys(log.values_by_path).filter((key) => key.startsWith('response.')).length +
                (log.unset_paths || []).filter((key) => key.startsWith('response.')).length,
            0
        );
        expect(logRows.length).toEqual(expectedCount);
    ...
    test('Test with an event of type widget_interaction with user action', async () => {
        ...
        expect(currentLog).toEqual(expect.objectContaining({ ...commonInterviewDataInRows, event_type: 'widget_interaction' }));
        const modifiedKeys = Object.entries(log.values_by_path)
            .filter(([key, value]) => value !== null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        const initializedKeys = Object.entries(log.values_by_path)
            .filter(([key, value]) => value === null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        ...
        expect(currentLog.unsetFields).toEqual(
            log.unset_paths !== undefined ? log.unset_paths.filter((path: string) => !path.startsWith('validations.')).join('|') : ''
        );
    ...
    describe('Tests with button_click events', () => {
        const testCases = [
            {
                ...
                unset_paths: ['response.home.someField', 'validations.home.someField'],
                expectedOutput: { hiddenWidgets: '', invalidFields: '' }
            },
            {
                ...
                expectedOutput: { hiddenWidgets: 'hiddenWidget1|hiddenWidget2', invalidFields: '' }
            },
            {
                ...
                values_by_path: { 'response.home.geography': { type: 'Point', coordinates: [1, 1] } },
                unset_paths: ['response.home.someField'],
                expectedOutput: { hiddenWidgets: '', invalidFields: 'invalidWidget1|invalidWidget2' }
            },
            {
                description: 'invalid widgets in user action and values_by_path',
                userAction: { type: 'buttonClick', buttonId: 'response.otherField', invalidWidgets: ['invalidWidget1', 'invalidWidget2'] },
                values_by_path: { 'response.home.geography': { type: 'Point', coordinates: [1, 1] }, 'validations.valuesByPathInvalid': false },
                unset_paths: ['response.home.someField'],
                expectedOutput: { hiddenWidgets: '', invalidFields: 'valuesByPathInvalid|invalidWidget1|invalidWidget2' }
            }
        ];
        ...
        test.each(testCases)('Test with $description', async ({ userAction, values_by_path, unset_paths, expectedOutput }) => {
            ...
            // Test the row values
            const modifiedKeys = Object.entries(values_by_path)
                .filter(([key, value]) => value !== null && !key.startsWith('validations.'))
                .map(([key, value]) => key)
                .join('|');
            const initializedKeys = Object.entries(values_by_path)
                .filter(([key, value]) => value === null && !key.startsWith('validations.'))
                .map(([key, value]) => key)
                .join('|');
    ...
    test('Test with an event of type section_change with user action', async () => {
        ...
        const userActionWithHidden = {
            type: 'sectionChange',
            targetSection: { sectionShortname: 'someSection', iterationContext: ['person', 'personId'] },
            previousSection: { sectionShortname: 'prevSection' },
        ...
        // Test the row values
        const modifiedKeys = Object.entries(sectionChangeLogs[0].values_by_path)
            .filter(([key, value]) => value !== null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        const initializedKeys = Object.entries(sectionChangeLogs[0].values_by_path)
            .filter(([key, value]) => value === null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        expect(logRows[0]).toEqual({
            ...
            initializedFields: initializedKeys,
            unsetFields:
                sectionChangeLogs[0].unset_paths !== undefined
                    ? sectionChangeLogs[0].unset_paths.filter((path: string) => !path.startsWith('validations.')).join('|')
                    : '',
            widgetType: '',
        ...
        const modifiedKeys2 = Object.entries(sectionChangeLogs[1].values_by_path)
            .filter(([key, value]) => value !== null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        const initializedKeys2 = Object.entries(sectionChangeLogs[1].values_by_path)
            .filter(([key, value]) => value === null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        expect(logRows[1]).toEqual({
            ...
            unsetFields:
                sectionChangeLogs[1].unset_paths !== undefined
                    ? sectionChangeLogs[1].unset_paths.filter((path: string) => !path.startsWith('validations.')).join('|')
                    : '',
            widgetType: '',
    ...
    test('Test with an event of type language_change with user action', async () => {
        ...
        // Test the row values
        const modifiedKeysLog1 = Object.entries(languageLogs[0].values_by_path)
            .filter(([key, value]) => value !== null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        const initializedKeysLog1 = Object.entries(languageLogs[0].values_by_path)
            .filter(([key, value]) => value === null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        expect(logRows[0]).toEqual({
            ...
            unsetFields:
                languageLogs[0].unset_paths !== undefined
                    ? languageLogs[0].unset_paths.filter((path: string) => !path.startsWith('validations.')).join('|')
                    : '',
            widgetType: '',
            widgetPath: '',
            hiddenWidgets: '',
    ...
    test('Test with an event of type interview_open with user action', async () => {
        ...
        // Test the row values
        const modifiedKeysLog1 = Object.entries(interviewOpenLogs[0].values_by_path)
            .filter(([key, value]) => value !== null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        const initializedKeysLog1 = Object.entries(interviewOpenLogs[0].values_by_path)
            .filter(([key, value]) => value === null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        expect(logRows[0]).toEqual({
    ...
    test('Test with various events, with validations `true` and `false`', async () => {
        ...
        // Test the row values
        const modifiedKeysLog1 = Object.entries(interviewLogs[0].values_by_path)
            .filter(([key, value]) => value !== null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        const initializedKeysLog1 = Object.entries(interviewLogs[0].values_by_path)
            .filter(([key, value]) => value === null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        expect(logRows[0]).toEqual({
            ...
            initializedFields: initializedKeysLog1,
            unsetFields:
                interviewLogs[0].unset_paths !== undefined
                    ? interviewLogs[0].unset_paths.filter((path: string) => !path.startsWith('validations.')).join('|')
                    : '',
            widgetType: '',
        ...
        const modifiedKeysLog2 = Object.entries(interviewLogs[1].values_by_path)
            .filter(([key, value]) => value !== null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        const initializedKeysLog2 = Object.entries(interviewLogs[1].values_by_path)
            .filter(([key, value]) => value === null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        expect(logRows[1]).toEqual({
        ...
        const modifiedKeysLog3 = Object.entries(interviewLogs[2].values_by_path)
            .filter(([key, value]) => value !== null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
        const initializedKeysLog3 = Object.entries(interviewLogs[2].values_by_path)
            .filter(([key, value]) => value === null && !key.startsWith('validations.'))
            .map(([key, value]) => key)
            .join('|');
    ...
    describe('Test with various context data', () => {
        ...
        const expectedRowFromLog = (log: { [key: string]: any }, overrides: { [key: string]: any } = {}) => ({
            ...
            initializedFields: log.values_by_path
                ? Object.entries(log.values_by_path)
                    ...
                    .join('|')
                : '',
            unsetFields: log.unset_paths ? log.unset_paths.filter((path: string) => !path.startsWith('validations.')).join('|') : '',
            widgetType: '',
            widgetPath: '',
            hiddenWidgets: '',
        ...
        const platformData1 = {
            parsedResult: { browser: { name: 'Firefox', version: '146.0' }, os: { name: 'Linux' }, platform: { type: 'desktop' } }
        };
        const platformData2 = {
            parsedResult: { browser: { name: 'Safari', version: '146.0' }, os: { name: 'MacOs' }, platform: { type: 'mobile' } }
        };
        ...
        const contextCases = [
            {
                ...
                logs: [
                    ...
                    makeLog({
                        ...
                        values_by_path: {
                            'validations.home.geography': true,
                            'validations.home.household.size': false,
                            'validations.home.someField': false
                        },
                        unset_paths: ['response.home.someField'],
                        user_action: { type: 'buttonClick', buttonId: 'response.someField' }
                    }),
            ...
            },
            {
                ...
                expectedRowOverrides: [
                    ...
                    { browser: '', os: '', platform: '', widgetPath: 'response.someField', user_id: '1', for_correction: 'true' },
                    {
                        browser: platformData2.parsedResult.browser.name,
                        os: platformData2.parsedResult.os.name,
                        platform: platformData2.parsedResult.platform.type,
            ...
            },
            {
                ...
                expectedRowOverrides: [
                    { language: 'fr' },
                    { language: 'es', user_id: '1' },
                    { language: '', widgetPath: 'response.someField', user_id: '1', for_correction: 'true' },
                    { language: 'es', widgetPath: 'response.someField', user_id: '1' },
                    { language: 'fr', widgetPath: 'response.someField' },
                    { language: 'fr', user_id: '1', for_correction: 'true' },
                    { language: 'es', widgetPath: 'response.someField', user_id: '1' }
                ]
            }
        ];
        ...
        test.each(contextCases)('Test with $description', async ({ logs, expectedRowOverrides }) => {
            ...
            for (let i = 0; i < expectedRowOverrides.length; i++) {
                const expectedRow = expectedRowFromLog(logs[i], expectedRowOverrides[i]);
