...
jest.mock('../../../models/interviewsPreFill.db.queries', () => ({ getByReferenceValue: jest.fn(), setPreFilledResponseForRef: jest.fn() }));
...
const mockSetPreFilledResponseForRef = prefilledDbQueries.setPreFilledResponseForRef as jest.MockedFunction<
    typeof prefilledDbQueries.setPreFilledResponseForRef
>;
...
const interviewAttributes = {
    ...
    response: { accessCode: '11111', testFields: { fieldA: 'a', fieldB: 'b' } },
    validations: {},
    survey_id: 1
} as any;
...
const updateCallbacks = [
    {
        ...
        callback: jest
            .fn()
            .mockImplementation((interview, fieldValue) =>
                fieldValue === 'foo'
...
describe('Simple field update', () => {
    each([
        ...
        [
            'Field set to specific value, should return an update',
            { 'response.testFields.fieldA': 'foo' },
            [],
            0,
            { 'response.testFields.fieldB': 'bar' }
        ],
        [
            'Field set to specific value in corrected_response, should return an update',
            { 'corrected_response.testFields.fieldA': 'foo' },
            [],
            0,
            { 'corrected_response.testFields.fieldB': 'bar' }
        ],
        ...
        [
            'Field update in corrected_response, should not run for this callback',
            { 'corrected_response._isCompleted': false },
            [],
            false,
            {},
            undefined
        ...
        async (
            ...
            valuesByPath,
            unsetPath,
            called: number | false = false,
            expectedFieldValues: { [path: string]: unknown } = {},
            expectedUrl = undefined
        ) => {
            expect(await updateServerFields(interviewAttributes, updateCallbacks, valuesByPath, unsetPath, deferredUpdateCallback)).toEqual([
                expectedFieldValues,
                expectedUrl
            ]);
            if (called !== false) {
                ...
                if (called === 0) {
                    expect(updateCallbacks[called].callback).toHaveBeenCalledWith(
                        interviewAttributes,
                        valuesByPath['response.testFields.fieldA'] !== undefined
                            ? valuesByPath['response.testFields.fieldA']
                            : valuesByPath['corrected_response.testFields.fieldA'],
                        'testFields.fieldA',
                        expect.anything()
                    );
                } else if (called === 2) {
                    expect(updateCallbacks[called].callback).toHaveBeenCalledWith(
                        interviewAttributes,
                        valuesByPath['response._isCompleted'],
                        '_isCompleted',
                        expect.anything()
                    );
                }
...
describe('Field with placeholder and regex', () => {
    test('Field in valuesByPath, should return updated value', async () => {
        ...
        expect(await updateServerFields(interviewAttributes, updateCallbacks, valuesByPath, [], deferredUpdateCallback)).toEqual([
            { 'response.household.persons.1.new': 'FOO' },
            undefined
        ]);
        expect(updateCallbacks[1].callback).toHaveBeenCalledWith(
            interviewAttributes,
            valuesByPath['response.household.persons.1.origin'],
            'household.persons.1.origin',
            expect.anything()
        );
    });
    ...
    test('Field in unsetPath, should return updated value', async () => {
        ...
        expect(await updateServerFields(interviewAttributes, updateCallbacks, {}, unsetPath)).toEqual([
            { 'response.household.persons.2.new': 'FOO' },
            undefined
        ]);
...
describe('With an update execution callback registration', () => {
    test('One execution callback call', async () => {
        ...
        expect(updateCallbacks[3].callback).toHaveBeenCalledWith(
            interviewAttributes,
            valuesByPath['response.testFields.callback'],
            'testFields.callback',
            expect.anything()
        );
    ...
    test('Multiple calls to same execution, at the same time', async () => {
        ...
        expect(deferredUpdateCallback).toHaveBeenCalledWith(
            Object.keys(call1Response).reduce((acc, key) => {
...
describe('getPreFilledValuesByPath', () => {
    ...
    test('Return fields that are not in the interview', async () => {
        const preFilledResponse = { 'testFields.fieldC': { value: 'abc' }, otherField: { value: 3 } };
