...
const participant1 = { email: 'test@test.org', username: 'p1', preferences: { lang: 'fr' }, first_name: 'Toto' };

const participant2 = { email: 'test@example.org', username: 'p2', preferences: { lang: 'fr' }, first_name: 'Toto' };

const participantToInsert = { email: 'newUser@test.org', preferences: { lang: 'fr' }, first_name: 'Foo' };
...
const participantIds = { participantId1: -1, participantId2: -1 };
...
test('Create new participant with duplicate key', async () => {
    await expect(dbQueries.create(participantToInsert)).rejects.toThrow(expect.anything());
});
...
describe('logLastLogin', () => {
    test('Correctly log last login', async () => {
        ...
        const newLastLogin = await knex('sv_participant_logins')
        ...
        const newLastLogin2 = await knex('sv_participant_logins')
            .select(['*', knex.raw('EXTRACT(EPOCH FROM timestamp) as unix_timestamp')])
            .orderBy('timestamp');
    ...
    test('Insert for an unexisting participant', async () => {
        await expect(dbQueries.logLastLogin(participantIds.participantId1 + 1000)).rejects.toThrow(expect.anything());
    });
});
