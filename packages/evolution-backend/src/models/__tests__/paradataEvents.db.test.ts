...
const secondParticipant = { id: 11, email: 'test1@transition.city', is_valid: true };

const localUser = { ...localParticipant, id: 2, uuid: uuidV4() };
...
const testInterviewAttributes2 = {
    ...
    is_active: true,
    is_completed: undefined,
    response: { _isCompleted: true, accessCode: '11111', booleanField: true },
    validations: {}
} as any;
...
describe('paradata log', () => {
    ...
    const validateLogData = async (expected: { userId: number | null; eventData: any; forCorrection?: boolean }[]) => {
        ...
        const events = await knex('paradata_events')
            .select(['*', knex.raw('EXTRACT(EPOCH FROM timestamp) as unix_timestamp')])
            .orderBy('timestamp');
    ...
    test('Log for a participant (no user id)', async () => {
        expect(await dbQueries.log({ interviewId: testInterviewAttributes1.id, eventType: 'button_click', eventData: defaultEventData })).toEqual(
            true
        );
    ...
    test('Log undefined event data', async () => {
        expect(await dbQueries.log({ interviewId: testInterviewAttributes1.id, eventType: 'button_click', userId: localUser.id })).toEqual(true);
        await validateLogData([{ userId: localUser.id, eventData: null }]);
    });

    test.each([[true], [false]])('Log with forCorrection set to `%s`', async (forCorrection) => {
        expect(
            await dbQueries.log({
                interviewId: testInterviewAttributes1.id,
                eventType: 'button_click',
    ...
    test('Log multiple data', async () => {
        ...
        const promises = [
            dbQueries.log({ interviewId: testInterviewAttributes1.id, eventType: 'button_click', eventData: defaultEventData, userId: localUser.id }),
            dbQueries.log({ interviewId: testInterviewAttributes1.id, eventType: 'button_click', eventData: secondEventData })
        ];
    ...
    each([
        ...
        const invalidParadataLog = Object.assign(
            { interviewId: testInterviewAttributes1.id, eventType: 'button_click', eventData: defaultEventData },
            changedData
        );
...
const insertSomeLogs = async (
    logData: (Record<string, any> | undefined)[],
    interviewId: number,
    userId?: number | undefined,
    forCorrection?: boolean | undefined
) => {
...
describe('Stream paradata', () => {
    ...
    test('Stream interview logs, only one interview has logs', (done) => {
        ...
        const logData = [
            ...
            { valuesByPath: {}, unsetPaths: ['response.data'] },
            undefined,
            { unsetPaths: ['response.data.someField', 'validations.data.someField'] }
        ];

        insertSomeLogs(logData, testInterviewAttributes1.id)
            .then(() => {
                let nbLogs = 0;
                let lastTimestamp = -1;
                const queryStream = dbQueries.getParadataStream();
    ...
    describe('with forCorrection filter', () => {
        ...
        const unknownResponseEvent = { eventType: 'legacy', valuesByPath: {}, unsetPaths: ['response.data'] };
        ...
        test('Stream interview with forCorrection set to `false`, should return participant and null', (done) => {
            ...
            const expectedLogs = [participantEvent, interviewerEvent, unknownResponseEvent];
        ...
        test('Stream interview with forCorrection set to `true`, should return corrected and null', (done) => {
            ...
            // This is the expected logs for the participant data
            const expectedLogs = [correctionEvent, unknownResponseEvent];
        ...
        test('Stream interview with forCorrection set to `true` and interviewId set', (done) => {
            ...
            insertSomeLogs([correctionEvent, unknownResponseEvent], testInterviewAttributes2.id, localUser.id, true)
                .then(() => {
                    let nbLogs = 0;
                    let lastTimestamp = -1;
...
describe('Query paradata temp view', () => {
    ...
    completeInterviewAttributes.response = { ...completeInterviewAttributes.response, _completedAt: new Date().toISOString(), _isCompleted: true };
    ...
    describe('getIncompleteInterviewsLastEventCounts', () => {
        ...
        test('Should return count only for incomplete interviews', async () => {
            ...
            const logData = [
                ...
                { eventType: 'server_event', valuesByPath: {}, unsetPaths: ['response.data'] },
                { eventType: 'side_effect', unsetPaths: ['response.data.someField', 'validations.data.someField'] }
            ];
