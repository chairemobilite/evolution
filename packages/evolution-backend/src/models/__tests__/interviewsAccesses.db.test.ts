...
const localUser = { ...localParticipant, uuid: uuidV4(), permissions: { [permission1]: true } };

const anotherUser = { id: 2, email: 'test2@transition.city', is_valid: true, uuid: uuidV4(), permissions: { [permission2]: true } };
...
describe('userOpenedInterview', () => {
    ...
    test('Interview does not exist', async () => {
        ...
        expect((exception as any).message).toEqual(
            expect.stringContaining('cannot log the user opening the interview (knex error: The requested interview does not exist:')
        );
        await assertAccessCounts(1);
    });
    ...
    test('User does not exist', async () => {
        ...
        expect(exception).toBeDefined();
        expect((exception as any).message).toEqual(
            expect.stringContaining('violates foreign key constraint "sv_interviews_accesses_user_id_foreign")')
        );
        await assertAccessCounts(1);
    });
    ...
    test('Second user opened existing interview', async () => {
        expect(
            await dbQueries.userOpenedInterview({ interviewUuid: localUserInterviewAttributes.uuid, userId: anotherUser.id, validationMode: false })
        ).toEqual(true);
    ...
    test('Second user opened existing interview, for validation', async () => {
        expect(
            await dbQueries.userOpenedInterview({ interviewUuid: localUserInterviewAttributes.uuid, userId: anotherUser.id, validationMode: true })
        ).toEqual(true);
...
describe('userUpdatedInterview', () => {
    ...
    test('User updated interview in edit mode, again', async () => {
        expect(
            await dbQueries.userUpdatedInterview({ interviewUuid: localUserInterviewAttributes.uuid, userId: anotherUser.id, validationMode: false })
        ).toEqual(true);
        await assertAccessCounts(3);
    ...
    test('User updated interview in validation mode', async () => {
        expect(
            await dbQueries.userUpdatedInterview({ interviewUuid: localUserInterviewAttributes.uuid, userId: anotherUser.id, validationMode: true })
        ).toEqual(true);
        await assertAccessCounts(3);
    ...
    test('New user updated interview in validation mode', async () => {
        expect(
            await dbQueries.userUpdatedInterview({ interviewUuid: localUserInterviewAttributes.uuid, userId: localUser.id, validationMode: true })
        ).toEqual(true);
    ...
    test('Interview does not exist', async () => {
        ...
        expect(exception).toBeDefined();
        expect((exception as any).message).toEqual(
            expect.stringContaining('cannot log the user updating an interview (knex error: The requested interview does not exist:')
        );
    });
