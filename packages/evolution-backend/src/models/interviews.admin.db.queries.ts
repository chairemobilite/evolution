 ...
 * prefixes (`telephone_`, `anonym_`), then email. A remaining username is
 * `byField` when that auth method is enabled. Otherwise `unknown`.
 */
export const getParticipantLoginMethodSelect = () => {
    const byFieldWhen = isByFieldAuthEnabled() ? 'WHEN participant.username IS NOT NULL THEN \'byField\'' : '';
    return knex.raw(
        `
        CASE
            WHEN participant.google_id IS NOT NULL THEN 'google'
            WHEN starts_with(participant.username, ?) THEN 'interviewer'
        ...
        END AS "loginMethod"
        `,
        [`${INTERVIEWER_PARTICIPANT_PREFIX}_`, `${ANONYMOUS_PARTICIPANT_PREFIX}_`]
    );
};
