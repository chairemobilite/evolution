...
/**
 * Click the next page button when not all options are filled and check that we remain on the same page.
 *
 * @param {Object} options - The options for the test.
 ...
 */
export const tryToContinueOnEmptyPage: ContinueWithInvalidEntriesTest = ({ context, text, currentPageUrl, nextPageUrl }) => {
    test(`Clicking ${text} when options are empty should keep you on ${currentPageUrl} - ${getTestCounter(context, `${text} - ${currentPageUrl} - ${nextPageUrl}`)}`, async () => {
...
export const tryToContinueWithPopup: ContinueWithInvalidEntriesTest = ({ context, text, currentPageUrl, nextPageUrl }) => {
    test(`Clicking ${text} when age is under 18 should open a popup and keep you on ${currentPageUrl} - ${getTestCounter(context, `${text} - ${currentPageUrl} - ${nextPageUrl}`)}`, async () => {
        await tryToContinueOnInvalidPage({ context, text, currentPageUrl, nextPageUrl });
        const agePopup = context.page.getByRole('dialog');
        await expect(agePopup).toBeVisible();
        await agePopup.locator("button").click();
        await await expect(agePopup).not.toBeVisible();
    });
};
