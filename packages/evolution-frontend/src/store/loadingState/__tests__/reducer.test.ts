...
test('Test increment loading state', () => {
    const action = { type: LoadingStateActionTypes.INCREMENT_LOADING_STATE };

    const result = { loadingState: 2 };

    expect(loadingStateReducer({ loadingState: 1 }, action)).toEqual(result);
});

test('Test decrement loading state', () => {
    const action = { type: LoadingStateActionTypes.DECREMENT_LOADING_STATE };

    const result = { loadingState: 1 };
