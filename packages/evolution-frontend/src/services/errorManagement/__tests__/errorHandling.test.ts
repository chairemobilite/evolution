...
describe('logClientSideMessage', () => {
    ...
    each([
        ['Error object and interview id', new Error('Test error'), 1, undefined, { message: 'Test error', interviewId: 1, logLevel: 'error' }],
        [
            'Error object, no interview id',
            new Error('Test error'),
            undefined,
            undefined,
            { message: 'Test error', interviewId: undefined, logLevel: 'error' }
        ],
        [
            'String error and interview id',
            'This is a string error',
            1,
            undefined,
            { message: 'This is a string error', interviewId: 1, logLevel: 'error' }
        ],
        ['Number error and interview id', 3, 1, undefined, { message: '3', interviewId: 1, logLevel: 'error' }],
        [
            'Object error and interview id',
            { a: 'This is not an error!', b: 'Yes, this is', c: 'We accept you as you are' },
            1,
            undefined,
            { message: '{"a":"This is not an error!","b":"Yes, this is","c":"We accept you as you are"}', interviewId: 1, logLevel: 'error' }
        ],
        [
            'Array error and interview id',
            ['This is a log message', 'then another', { a: '2' }],
            1,
            undefined,
            { message: 'This is a log messagethen another[object Object]', interviewId: 1, logLevel: 'error' }
        ],
        ['Undefined error and interview id', undefined, 1, undefined, { message: 'undefined', interviewId: 1, logLevel: 'error' }],
        [
            'Error object, interview id, log level error',
            new Error('Test error'),
            1,
            'error',
            { message: 'Test error', interviewId: 1, logLevel: 'error' }
        ],
        [
            'Undefined error, interview id, log level warn',
            new Error('Test error'),
            1,
            'warn',
            { message: 'Test error', interviewId: 1, logLevel: 'warn' }
        ],
        [
            'Undefined error, no interview id, log level log',
            new Error('Test error'),
            undefined,
            'log',
            { message: 'Test error', interviewId: undefined, logLevel: 'log' }
        ]
    ...
    test('With only one argument', async () => {
        ...
        expect(fetchSpy).toHaveBeenCalledWith('/api/survey/logClientSideMessage', {
            ...
            method: 'POST',
            credentials: 'include',
            body: JSON.stringify({ message: 'This is an error', interviewId: undefined, logLevel: 'error' })
        });
    });
});
...
describe('Console log overwriting', () => {
    ...
    test('Call to console.info with multiple arguments should report client side exception, no interview id getter', () => {
        ...
        expect(ErrorHandlingFct.logClientSideMessage).toHaveBeenCalledWith(['This is a log message', 'with other data', { a: '2', b: '3' }], {
