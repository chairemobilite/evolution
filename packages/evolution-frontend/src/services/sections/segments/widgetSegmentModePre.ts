...
const getModePreChoices = () => [
    ...
    {
        value: 'ferry',
        label: (t: TFunction) => t(['customSurvey:segments:mode:Ferry', 'segments:mode:Ferry']),
...
export const getModePreWidgetConfig = (
    ...
    return {
        ...
        label: (t: TFunction, interview, path) => {
            ...
            return sequence === 1
                ? t(['customSurvey:segments:PremierModeChoix', 'segments:PremierModeChoix'], {
                    context: options.context?.(),
                    originName,
                    destinationName,
                    count: odHelpers.getCountOrSelfDeclared({ interview, person })
                })
                : t(['customSurvey:segments:ModeChoixAutre', 'segments:ModeChoixAutre'], {
                    context: options.context?.(),
                    originName,
                    destinationName,
                    count: odHelpers.getCountOrSelfDeclared({ interview, person })
