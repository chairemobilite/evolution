...
import { modePreToModeMap, modeToModePreMap } from 'evolution-common/lib/services/odSurvey/types';
...
describe('Mode conditional', () => {
    ...
    const pathToCurrentSegment = 'household.persons.personId2.journeys.journeyId2.trips.tripId1P2.segments.segmentId1P2T1';
    setResponse(testInterview, `${pathToCurrentSegment}.modePre`, modePreWithMultipleChoices);
    ...
    test('should not be displayed by default', () => {
        ...
        setResponse(interview, `${pathToCurrentSegment}.modePre`, undefined);
        // Test conditional
        const result = conditional!(interview, `${pathToCurrentSegment}.mode`);
        expect(result).toEqual([false, null]);
    });

    test('should be displayed, if modePre is set and there is more than one choice', () => {
        // Test conditional
        const result = conditional!(testInterview, `${pathToCurrentSegment}.mode`);
        expect(result).toEqual([true, null]);
    });

    test('should not be displayed, if modePre is set and there is only one choice', () => {
        const interview = _cloneDeep(testInterview);
        // Set the segment's modePre to a mode that should resolve to multiple choices
        setResponse(interview, `${pathToCurrentSegment}.modePre`, modePreWithSingleChoice);
    ...
    test('should not be displayed if same mode as reverse trip and segment is new', () => {
        ...
        // Set a mode that does not exist to segment of previous trip, to make sure it is the one returned
        const mode = 'walkingMode';
        setResponse(interview, 'household.persons.personId2.journeys.journeyId2.trips.tripId1P2.segments.segmentId1P2T1.mode', mode);
        // Set same as previous trip to true nad _isNew to true
        ...
        // Test conditional
        const result = conditional!(interview, 'household.persons.personId2.journeys.journeyId2.trips.tripId2P2.segments.segmentId1P2T2.mode');
    ...
    test('should be displayed if same mode as reverse trip but segment is not new', () => {
        ...
        const mode = 'walkingMode';
        setResponse(interview, 'household.persons.personId2.journeys.journeyId2.trips.tripId1P2.segments.segmentId1P2T1.mode', mode);
        // Set same as previous trip to true and _isNew to false
        setResponse(interview, 'household.persons.personId2.journeys.journeyId2.trips.tripId2P2.segments.segmentId1P2T2.sameModeAsReverseTrip', true);
        ...
        setResponse(interview, 'household.persons.personId2.journeys.journeyId2.trips.tripId2P2.segments.segmentId1P2T2.modePre', modePreWithMultipleChoices); 
    ...
    test('should be displayed if same mode as reverse trip, but no previous trip', () => {
        ...
        setResponse(interview, `${pathToCurrentSegment}.sameModeAsReverseTrip`, true);
        setResponse(interview, `${pathToCurrentSegment}.modePre`, modePreWithMultipleChoices);
