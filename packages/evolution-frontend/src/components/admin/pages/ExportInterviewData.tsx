...
const ExportInterviewData = ({ t, i18n }: WithTranslation) => {
    ...
    const downloadSurveyQuestionnaireListTxt = async () => {
        try {
            ...
            const url = `/api/admin/data/downloadSurveyQuestionnaireListTxt?lang=${lang}`;
            const response = await fetch(`/api/admin/data/downloadSurveyQuestionnaireListTxt?lang=${lang}`, {
                method: 'GET',
                credentials: 'include',
                headers: {
                    'Content-Type': 'application/json'
