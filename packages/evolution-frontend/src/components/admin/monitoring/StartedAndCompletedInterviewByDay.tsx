...
type StartedAndCompletedInterviewsByDayProps = {
    onUpdate: () => void;
    lastUpdateAt?: number;
} & WithTranslation;

class StartedAndCompletedInterviewsByDay extends React.Component<
    StartedAndCompletedInterviewsByDayProps,
    StartedAndCompletedInterviewsByDayState
> {
    constructor(props: StartedAndCompletedInterviewsByDayProps) {
        super(props);
        this.state = {
            data: {
                started: [],
    ...
    render() {
        ...
        this.state.data.started.reduce((cumulativeCount, value, i) => {
            return (cumulativeStartedData[i] = cumulativeCount + value);
        }, 0);
        const cumulativeCompletedData: number[] = [];
        this.state.data.completed.reduce((cumulativeCount, value, i) => {
            return (cumulativeCompletedData[i] = cumulativeCount + value);
        }, 0);
        const dataMax: number = _max(this.state.data.started) || 0;
        ...
        const chartOptions = {
            ...
            tooltip: {
                ...
                pointFormat:
                    '<span style="color: {point.color}">{point.name}</span> <strong>{point.y:.0f}</strong><br />'
            },
            series: [
                {
