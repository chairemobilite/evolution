...
test('Test simple modal with action on close', () => {
    ...
    const { queryByText } = render(
        <SimpleModal isOpen={true} closeModal={handleClose} text={text} title={title} containsHtml={true} action={action} />
    );
...
test('Test simple modal with minimal parameters', () => {
    ...
    const title = 'Simple modal title';
    const { getByText } = render(<SimpleModal isOpen={true} closeModal={handleClose} text={text} title={title} />);
