...
// Mock db queries
jest.mock('../maps/google/GoogleGeocoder', () => ({ geocodeMultiplePlaces: jest.fn() }));
        ...
        const testWidgetConfig = Object.assign(
            {
                ...
                refreshGeocodingLabel: { fr: 'Rafraîchir la carte', en: 'Refresh map' },
                icon: { url: 'path/to/icon', size: [80, 80] as [number, number] },
                maxZoom: 18,
                defaultZoom: 15,
                coordinatesPrecision: 6,
                placesIcon: { url: 'path/to/places-icon', size: [85, 85] as [number, number] },
                selectedIcon: { url: 'path/to/selected-icon', size: [90, 90] as [number, number] },
                maxGeocodingResultsBounds: function (interview, path) {
                    return [
                        { lat: 45.2229, lng: -74.323 },
                        { lat: 46.1181, lng: -72.9215 }
                    ] as [{ lat: number; lng: number }, { lat: number; lng: number }];
                },
                invalidGeocodingResultTypes: ['political', 'country'],
    ...
    const testWidgetConfig = Object.assign(
        {
            ...
            refreshGeocodingLabel: { fr: 'Geocode', en: 'Geocode' },
            icon: { url: 'path/to/icon' },
            maxZoom: 18,
            defaultZoom: 15,
            placesIcon: { url: 'path/to/icon' },
            size: 'medium',
            updateDefaultValueWhenResponded: true
        },
        baseWidgetConfig
