...
const interview = { id: 1, uuid: '', participant_id: 1, is_completed: false, response: {}, validations: {}, is_valid: true };
...
const baseWidgetConfig = { type: 'question' as const, label: 'test', path: 'test.radioNumber' };

describe('Render InputRadioNumber', () => {
    const widgetConfig = { ...baseWidgetConfig, inputType: 'radioNumber' as const, valueRange: { min: 1, max: 3 }, overMaxAllowed: false };

    const widgetConfigOverMax = { ...baseWidgetConfig, inputType: 'radioNumber' as const, valueRange: { min: 1, max: 3 }, overMaxAllowed: true };

    test('InputRadioNumber without "over max" option', () => {
        const { container } = render(
            <InputRadioNumber id={'test'} widgetConfig={widgetConfig} onValueChange={(e) => null} interview={interview} path={''} user={user} />
        );
...
describe('InputRadioNumber onChange', () => {
    const widgetConfig = { ...baseWidgetConfig, inputType: 'radioNumber' as const, valueRange: { min: 0, max: 3 }, overMaxAllowed: true };
