...
describe('Should correctly render InputTime with various parameters', () => {
    ...
    test('Test with min max times values', () => {
        ...
        const widgetConfig = Object.assign(
            { minTimeSecondsSinceMidnight: 10 * 60 * 60, maxTimeSecondsSinceMidnight: 12 * 60 * 60 },
            baseWidgetConfig
        );
        const { container } = render(
            <InputTime
    ...
    test('Test with min max times and steps functions', () => {
        ...
        const widgetConfig = Object.assign(
            { minTimeSecondsSinceMidnight: minTimesFct, maxTimeSecondsSinceMidnight: maxTimesFct, minuteStep: stepFct },
            baseWidgetConfig
        );
        const { container } = render(
            <InputTime
    ...
    test('Test with suffixTimes, minutes steps and hour separator', () => {
        ...
        const suffixTimeFct = jest.fn().mockReturnValue({ [(10 * 60 * 60).toString()]: ' 10 suffix', [(13 * 60 * 60).toString()]: ' 13 suffix' });
        // have all times, at 15 minutes interval, separator between hours and some suffixes
        const widgetConfig = Object.assign({ minuteStep: 15, suffixTimes: suffixTimeFct, addHourSeparators: true }, baseWidgetConfig);
        const { container } = render(
            <InputTime
                id={'test'}
                onValueChange={() => {
