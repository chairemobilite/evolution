...
type CustomSurvey = { section1?: { q1?: string; q2?: number }; section2?: { q1?: string } };
...
each([
    ...
    [
        'multiple groups: all valid',
        jest.fn().mockReturnValue([
            { validation: false, errorMessage: 'error' },
            { validation: false, errorMessage: 'error2' }
        ]),
        [true, undefined]
    ],
    [
        'multiple groups: second is invalid',
        jest.fn().mockReturnValue([
            { validation: false, errorMessage: 'error' },
            { validation: true, errorMessage: 'error2' }
        ]),
        [false, 'error2']
    ],
    [
        'multiple groups: all invalid',
        jest.fn().mockReturnValue([
            { validation: true, errorMessage: 'error' },
            { validation: true, errorMessage: 'error2' }
        ]),
        [false, 'error']
    ],
    [
        'function with error, should be valid',
