...
def error_when_missing_required_fields(required_fields_names, required_fields_values, row_number: int):
    if any(field is None for field in required_fields_values):
        missing_fields = [field_name for field_value, field_name in zip(required_fields_values, required_fields_names) if field_value is None]
        raise Exception(
