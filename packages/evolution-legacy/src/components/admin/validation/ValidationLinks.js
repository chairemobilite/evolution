...
export default function({handleInterviewSummaryChange, updateValuesByPath, interviewIsValid, interviewUuid, user, nextInterviewUuid, prevInterviewUuid, refreshInterview, resetInterview, interviewIsComplete, interviewIsValidated, t}) {
  ...
  return (
    <p className="center _large">
      ...
      { <React.Fragment>{" "}&nbsp;&nbsp;<a href="#" className={`_red${interviewIsComplete === false ? ' _active-background' : ''}`}  title={t('admin:SetIncomplete')} onClick={(e) => { e.preventDefault(); updateValuesByPath({is_completed: false}); }}><span className="_small"><FontAwesomeIcon icon={faBan} /></span><FontAwesomeIcon icon={faClipboardList} /></a></React.Fragment> }
      { <React.Fragment>{" "}&nbsp;&nbsp;<a href="#" className={`_green${interviewIsComplete === true ? ' _active-background' : ''}`} title={t('admin:SetComplete')}   onClick={(e) => { e.preventDefault(); updateValuesByPath({is_completed: true}); }}><span className="_small"><FontAwesomeIcon icon={faCheck} /></span><FontAwesomeIcon icon={faClipboardList} /></a></React.Fragment> }
